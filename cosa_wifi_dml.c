    ...
    )
{
    ...
    PCOSA_DML_WIFI_SSID             pWifiSsid     = (PCOSA_DML_WIFI_SSID      )NULL;
    UCHAR                           PathName[64]  = {0};
	UCHAR InstanceNumber[2]={'\0'};
//	printf("\nSecurity_Commit");
//	printf("\nSecurity_Commit 2");
	printf("\npWifiAp->AP.Cfg.InstanceNumber=%d",pWifiAp->AP.Cfg.InstanceNumber);
    ...
    while ( pSLinkEntry )
    {
        ...
        sprintf(PathName, "Device.WiFi.SSID.%d.", pSSIDLinkObj->InstanceNumber);
        printf("\n%s==%s",PathName,pWifiAp->AP.Cfg.SSID);
	...
	sprintf(InstanceNumber,"%d",pWifiAp->AP.Cfg.InstanceNumber);

	#if !defined(_COSA_INTEL_USG_ATOM_) && !defined(_COSA_BCM_MIPS_)
        printf("\n%d %s",__LINE__,__func__);
        return CosaDmlWiFiApSecSetCfg((ANSC_HANDLE)pMyObject->hPoamWiFiDm, InstanceNumber, pWifiApSecCfg);
	#else
        printf("\n%d %s",__LINE__,__func__);
        return CosaDmlWiFiApSecSetCfg((ANSC_HANDLE)pMyObject->hPoamWiFiDm, InstanceNumber, pWifiApSecCfg);
	#endif
...
//	printf("\n\n%d %s",__LINE__,__func__);
	...
	{
//		printf("\ncfg->SSID=%s pWifiSsid->SSID.StaticInfo.Name=%s",pWifiSsid->SSID.Cfg.SSID,pWifiSsid->SSID.StaticInfo.Name);
		printf("\nPsLink Entry off");
	}
    ...
    if ( pSLinkEntry )
    {
	printf("\n\n PSLINK ENTRY IS ON",__LINE__,__func__);
